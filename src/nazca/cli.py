...
@click.option("--model", default=None, help="nano-banana (default,fast,ref) | nano-banana-2 (ref) | nano-banana-2-lite (cheapest,fastest, 1 ref) | nano-banana-pro (ref, legible text, 14 refs) | imagen-4 | imagen-4-fast | imagen-3 (t2i only) | gpt-image-2 (OpenAI; legible text/ads, ref up to 5)")
...
@click.option("--model", default=None, help="TTS model (default: atlas-tts-grok). Also: atlas-tts-elevenlabs-v3, worder-tts, fish-tts, elevenlabs-tts.")
@click.option("--voice", default=None, help="Named voice (model-specific; required for worder-tts — a voice_id from GET /api/v1/voices — fish-tts — a reference_id from GET /model — and elevenlabs-tts — a voice_id from GET /v2/voices).")
...
@click.option("--dry-run", is_flag=True, help="Write the planned request; no API call.")
def music(prompt, out, lyrics, output_format, model, dry_run):
    ...
    try:
        result = _generate_music(
            out, prompt, model=resolved_model, lyrics=lyrics,
            output_format=output_format, dry_run=dry_run,
        )
    except BackendError as e:  # 429s/auth/HTTP errors → clean one-liner, not a traceback
        _emit_backend_error(e)
...
@click.option("--text-file", "text_file", default=None, type=click.Path(exists=True), help="Read the transcript from a file instead of --text (longer transcripts).")
